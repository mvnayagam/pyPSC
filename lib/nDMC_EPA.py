...
from .x3Dlinearizazion import linearizenD_EPA
...
# def isosurfs_EPA(h, xexp, f, fname):
#     n  = [] 
#     if h <= 2:
#         for l in range(1,h+1):
#             gi = g(l, xexp, f)
#             normal, dist, pwaste = linearizenD_EPA(l, f, np.abs(gi))
            
#             n.append([l, normal, dist, np.sign(gi)])
#             fname.write("%3g\t%2.6f\t%2.6f\t%2.6f\t%2.6f\t%2.6f\n"%(h, normal[0],normal[1],normal[2], dist[0],dist[1]))
#     else:
#         gi = g(h,xexp,f)
#         normal, dist, pwaste = linearizenD_EPA(h, f, np.abs(gi))
...
def nDMC_EPA_v1(dimension: int, noofpair: int, structure: list = [], noofRO: int=9, iorg: str='amplitude', imax=0.5) -> None:
    ...
    for h in range(2,noofRO+1):
        ...
        for rc, pairs in enumerate(rp):
            ...
            print("\x1b[1;34m===> Pair-%2g: "%(rc+1),"Current pair : \x1b[0m", pairs, xexp)
            ...
            nar = isosurfs_EPA(h, xexp, f, fstep1)
            fstep1.close()
            ...
            if h <= 2:
                for i in nar: # here i[0]->l,i[1]->normal,i[2]->distance,i[3]->amplitudesign
                    ...
                    #print(f'\nlen(polytope) : {len(o)} this should be equal to 2*l**m : {2*i[0]**dimension}')
                    if i[0]==1:
            ...
            print(f" --- L(sol)= {len(solun)} s = {sel1} --- ", end=" ") 
            
            #rr=((3/4)*(1/np.pi)*(volAsym))**(1/3)
            rr = radius_from_volume(dim=dimension, volume=volAsym)
            
            sortorder = np.argsort(pairs)[::-1]
            localmat  = pc.extreme(solun[sel1])#, abs_tol=1E-7)
            ...
            m2 = np.sort(np.array(0.5-m))[np.argsort(pairs).argsort()] #[np.argsort(pairs)[::-1].argsort()]
            
            print(f"---> predicted struc: {m} or {m2}. Given struc: {pairs}")
...
def nDMC_EPA(dimension: int, noofpair: int, structure: list = [], noofRO: int=9, iorg: str='amplitude',
    ...
    for h in range(hstart,noofRO+1):
        ...
        for rc, pairs in enumerate(rp):
            ...
            try:
                ...
                print(f'contains_coordinates: {contains_coordinates}')
            ...
            print(f" -> L(s): {len(solun)} s: {best_inx} <-", end=" ") 
