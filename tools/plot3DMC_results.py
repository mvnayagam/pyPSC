...
from psc.g_space import F, g, hsurf_F, hsurf_g, hsurf_F2
from psc.x3Dreadwrite import wrtcoor, wrtdata, readoldsolution, readh5pymean
...
def plot3dmcresults(h:int, fpath: str):
    ...
    pair = readh5pymean(fn)[4]   ## to get unsorted pair
    ...
    for ai, ls in enumerate(l):
        ...
        fn = os.path.join(fpath,'pnew_%g.h5'%(ls))
        allvol.append(readh5pymean(fn)[0])
        error.append(np.array(readh5pymean(fn)[2]))
        centero=np.array(readh5pymean(fn)[5])
        radi=np.array(readh5pymean(fn)[6])
        
        noofsolus=np.array(readh5pymean(fn)[7])
        ...
        for en, ei in enumerate(np.array(error[ai])):
            ...
            axs.flat[ai].set_xlim(0.,0.5,5)
            axs.flat[ai].set_ylim(0.,0.5,5)
            axs.flat[ai].set_zlim(0.,0.5,5)
